...
struct CursorHookPayload {
    ...
    error: Option<String>,
    permission: Option<CursorPermission>,
...
mod tests {
    ...
    fn test_parse_session_start() {
        ...
        let payload =
            r#"{"type":"session_start","session_id":"cursor-1","model":"claude-sonnet-4"}"#;
    ...
    fn test_parse_task_complete() {
        let plugin = CursorPlugin;
        let payload = r#"{"type":"done","session_id":"cursor-1","tokens_input":5000,"tokens_output":2500}"#;
        let result = plugin.parse(payload).unwrap().unwrap();
    ...
    fn test_parse_permission_request() {
        let plugin = CursorPlugin;
        let payload = r#"{"type":"permission_request","session_id":"cursor-1","permission":{"command":"write src/main.rs","description":"Create main file"}}"#;
        let result = plugin.parse(payload).unwrap().unwrap();
